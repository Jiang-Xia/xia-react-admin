...
export const handleLoginout = async (access_token) => {
    return $axios
        .post(config.smaiuac_service_base_url + '/api/v2/token/user/destroy_token', access_token)
        .then((res) => res.data)
}
// 修改密码的请求
export const handleResetpwd = async (data) => {
    return $axios
        .post(config.smaiuac_service_base_url + '/api/v1/user_manage/modify_selfpwd', data)
        .then((res) => res.data)
}
